...
constexpr std::int64_t kTileM = 256;
constexpr std::int64_t kTileN = 256;
constexpr std::int64_t kBakedK = 7168;
constexpr std::int64_t kExperts = 96;
constexpr std::int64_t kRowsPerExpert = 1024;
constexpr std::int64_t kValidRowsPerExpert = 1024;
constexpr std::int64_t kScaleAWmmaRep = 8;
...
torch::Tensor launch_moe_gemm1(
  ...
  TORCH_CHECK(m_per_expert == kTileM, "this launcher requires tile_m=256");
  ...
  const std::int64_t routed_rows = experts * kRowsPerExpert;
  ...
  TORCH_CHECK(
      ...
      "C stride1 mismatch");
  if (pipeline_contract) {
    const std::int64_t sa_size1 = contiguous_m / kScaleAWmmaRep;
    const std::int64_t sa_size2 = (k / 32) * kScaleAWmmaRep / 4;
    TORCH_CHECK(
  ...
  const std::int64_t m_tiles = contiguous_m / kTileM;
  const std::int64_t n_tiles = n / kTileN;
  const std::int64_t expected_grid_x = (m_tiles / 4) * n_tiles;
  const bool standard_grid =
      grid[0] == expected_grid_x && grid[1] == 4 && grid[2] == 1;
  const bool persistent_grid =
      grid[0] == 16 && grid[1] == 16 && grid[2] == 1;
  TORCH_CHECK(
      standard_grid || persistent_grid,
      "grid must be the standard MoE geometry or persistent (16,16,1)");
  TORCH_CHECK(
      block[0] == 128 && block[1] == 1 && block[2] == 1,
      "block must be (128,1,1)");
  TORCH_CHECK(
      cluster[0] == 4 && cluster[1] == 4 && cluster[2] == 1,
      "cluster must be (4,4,1)");
  ...
  attrs[0].val.clusterDim.x = checked_dim(cluster[0], "cluster.x");
  attrs[0].val.clusterDim.y = checked_dim(cluster[1], "cluster.y");
  attrs[0].val.clusterDim.z = checked_dim(cluster[2], "cluster.z");

  HIP_LAUNCH_CONFIG launch_config{};
  launch_config.gridDimX = checked_dim(grid[0], "grid.x");
  launch_config.gridDimY = checked_dim(grid[1], "grid.y");
  launch_config.gridDimZ = checked_dim(grid[2], "grid.z");
  launch_config.blockDimX = checked_dim(block[0], "block.x");
  launch_config.blockDimY = checked_dim(block[1], "block.y");
  launch_config.blockDimZ = checked_dim(block[2], "block.z");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def(
      "launch",
      &launch_moe_gemm1,
      "Launch the exact 256x256 ABpreShuffle MoE GEMM1 code object",
      py::arg("out"),
      py::arg("a"),
      py::arg("b"),
      py::arg("scale_a"),
