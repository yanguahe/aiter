...
def render_perf_counter_lines(
    ...
    visible_samples = [
        ...
        if axis_min <= ts - origin < axis_max
    ]
...
def print_interval_instruction_timeline(
    ...
    for lane_idx, lane in enumerate(lanes):
        ...
        for line in render_timeline_layer(boxes, axis_min, axis_max, scale, tick_position="top" if lane_idx == 0 else "bottom"):
            print(line)
    ...
    for version_idx, raw_version in enumerate(raw_versions):
        ...
        if representative_trace:
            ...
            trace_interval_idx = trace_indices.get(
                (representative_interval.wave_file, representative_interval.start_ts, representative_interval.start_pos),
                0,
            )
            print(f"\n################ kernel={version_name} ################")
            print(f"trace_dir={analysis.trace_dir}")
