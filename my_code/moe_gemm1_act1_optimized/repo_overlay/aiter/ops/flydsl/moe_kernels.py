...
def flydsl_moe_fused_quant_preshuffle(
    ...
    *,
    ...
    if topids_to_rows is not None:
        ...
        if producer not in ("three_kernel", "legacy"):
            raise ValueError(
                "AITER_FLYDSL_GEMM1_A_PRESHUFFLE_PRODUCER must be "
                f"three_kernel or legacy, got {producer!r}"
            )
        use_three_kernel_a_preshuffle = producer == "three_kernel"
