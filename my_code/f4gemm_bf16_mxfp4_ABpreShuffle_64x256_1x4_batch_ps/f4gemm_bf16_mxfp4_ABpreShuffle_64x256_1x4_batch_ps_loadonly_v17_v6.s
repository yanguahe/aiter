	...
	.text
	; ---------------------------------------------------------------------
	; v17_v6: K-split hardware-iterate payload loading, derived from v17.
	;
	; This file changes only the live four-wave A/B payload descriptors and
	; the descriptor operands required by iteration.  The kernel symbol, ABI,
	; single schedule, barriers/waits, four-stage ring, persistent scheduler,
	; K256 software recurrence, LDS allocation, and load-only/no-store contract
	; remain those of v17.  The source v17 file is intentionally untouched.
	;
	; Host shuffle_weight_f4 lays out packed bytes as
	;   [row_block16][logical_k_block32][row_in_block16][packed_k_byte16].
	; Consequently one M16xK32 or N16xK32 tile is 16*16 = 0x100 bytes,
	; and logical K128 is exactly four such tiles = physical byte +0x400.
	;
	; Per K256 phase (data_size encoding 0 => 1 byte):
	;   wave0 A: K[  0,128), tile_dim0=0x100, tile_dim1=4
	;   wave1 A: K[128,256), tile_dim0=0x100, tile_dim1=4
	;   wave2 B: K[  0,128), tile_dim0=0x100, tile_dim1=16
	;   wave3 B: K[128,256), tile_dim0=0x100, tile_dim1=16
	; All four use iterate_enable=1, iterate_count=3 (four executions), and
	; global_addr_increment=0x100 data_size elements.  Group 2 is s[80:83]
	; and group 3 is the zeroed s[84:87].  A uses lds_addr_increment=0x400
	; (0x100*4); B uses 0x1000 (0x100*16), so iterations are contiguous and
	; non-overlapping in LDS.
	;
	; Initial/current and selected next-task bases for waves 1 and 3 receive
	; physical +0x400, never logical-element +128.  Software still advances
	; every descriptor by +0x800 bytes per K256 phase; hardware iteration is
	; internal to one issue and does not replace or duplicate that phase step.
	;
	; Per ring slot, wave0/1 cover the original 0x2000-byte A payload as two
	; adjacent 0x1000-byte halves.  Wave2/3 cover the original 0x8000-byte B
	; payload as two adjacent 0x4000-byte halves.  The maxima remain 0x08000
	; for A and 0x2a800 for B, preserving the following scale/output regions.
	; ---------------------------------------------------------------------
	; ---------------------------------------------------------------------
	; LOAD-ONLY BANDWIDTH VARIANT of
	;   my_code/f4gemm_bf16_mxfp4_ABpreShuffle_64x256_1x4_batch_ps.s
	;
	; Keeps only the global->LDS TDM load pipeline for A, B, sA and sB: the
	; batch-Z pointer prologue, cluster/task decoding, persistent X/Y
	; scheduler, the four wave-specialised TDM descriptors, the four-slot LDS
	; ring, every tensor_load_to_lds with its descriptor recurrence, every
	; s_wait_tensorcnt throttle, every workgroup/cluster barrier, the K loop
	; and its termination, and the persistent-task restart.  Removed: all
	; v_wmma_scale_f32_32x16x128_f4, all ds_load/ds_store, the F32
	; accumulator clears and their s_call_i64 subroutine, the BF16
	; v_cvt_pk_bf16_f32 epilogue, the output tensor_store_from_lds and its
	; descriptor, the D-tile global_prefetch_b8, the per-lane LDS read
	; addresses, the s_prefetch_inst icache warm-up, and the now-vacuous
	; s_wait_dscnt / s_wait_alu / s_set_vgpr_msb bank switches.
	;
	; Nothing is written back, so the runner's correctness check fails by
	; construction and it exits with status 3; only the timing row is
	; meaningful.  Resource declarations (LDS 0x32800, 384 VGPR, 104 SGPR,
	; 120-byte kernarg ABI, symbol name) are unchanged so occupancy matches
	; the real kernel and gemm_batch_isa_runner.py still accepts the source.
	;
	; Run from the aiter repository root; run_loadonly_bw.sh in this same
	; directory carries the full vdi_mc procedure:
	;   AITER_LOG_MORE=1 python my_code/isa_runner/gemm_batch_isa_runner.py
	;       --iters 100 --shape 64,6144,7168 --batch 96
	;       --isa ./my_code/f4gemm_bf16_mxfp4_ABpreShuffle_64x256_1x4_batch_ps/f4gemm_bf16_mxfp4_ABpreShuffle_64x256_1x4_batch_ps_loadonly.s
	; ---------------------------------------------------------------------
	; Contract: batch-major A[B,M,K/2], B[B,N,K/2], sA[B,M,K/32],
	; sB[B,N,K/32], D[B,M,N]; wave M64xN64, WG M64xN256,
	; cluster (N x M x Z)=4x1x1, grid Z=batch, full K256 bodies.
	; One persistent X/Y scheduler is replicated per grid-Z batch plane.
	; The 120-byte preload ABI appends five u64 byte strides in D/A/B/sA/sB order.
...
.Lbranch_000000001da0:
	...
	s_add_co_ci_u32 s73, 0, s5
	s_mul_i32 s24, s55, 0x40
	s_mul_i32 s24, s24, s13
	s_add_co_u32 s76, s4, s24
	s_add_co_ci_u32 s77, 0, s5
	s_add_co_u32 s76, s76, 0x400                               ; wave1 physical K128
	s_add_co_ci_u32 s77, s77, 0
	s_mul_i32 s24, s54, 0x100
	s_mul_i32 s24, s24, s14
	s_add_co_u32 s74, s6, s24
	s_add_co_ci_u32 s75, 0, s7
	s_mul_i32 s24, s54, 0x100
	s_mul_i32 s24, s24, s14
	s_add_co_u32 s78, s6, s24
	s_add_co_ci_u32 s79, 0, s7
	s_add_co_u32 s78, s78, 0x400                               ; wave3 physical K128
	...
	s_cselect_b32 s24, s72, s24                                ; 000000001E64: 9818184A
	s_cselect_b32 s25, s73, s25                                ; 000000001E68: 9819194B
	s_cselect_b32 s26, s13, s26                                ; 000000001E6C: 981A1A0E
	s_cmp_eq_u32 s22, 2                                        ; 000000001E70: BF068216
	s_cselect_b32 s24, s74, s24                                ; 000000001E74: 9818184C
	s_cselect_b32 s25, s75, s25                                ; 000000001E78: 9819194D
	s_cselect_b32 s26, s14, s26                                ; 000000001E7C: 981A1A0F
	s_cmp_eq_u32 s22, 3                                        ; 000000001E80: BF068316
	s_cselect_b32 s24, s74, s24                                ; 000000001E84: 9818184E
	s_cselect_b32 s25, s75, s25                                ; 000000001E88: 9819194F
	s_cselect_b32 s26, s14, s26                                ; 000000001E8C: 981A1A10
	s_and_b32 s23, s22, 1
	s_cmp_eq_u32 s23, 0
...
.Lbranch_000000001f08:
	; v17_v6 wave0 A payload: K[0,128).
	...
	s_or_b32 s39, s39, 0x1000000                               ; 000000001FD0: 8C27FF27 08000000
	s_and_b32 s40, s40, 0xffff0000                             ; 000000001FD8: 8B28FF28 FFFF0000
	s_or_b32 s40, s40, 4                                      ; four M16 blocks, 0x400 B per iteration
	...
	; D# group2/group3: LDS +0x400, global +0x100, four iterations.
	s_bitset1_b32 s36, 19                              ; iterate_enable
	s_mov_b32 s80, 0                                   ; tensor_dim2
	s_mov_b32 s81, 0x400                             ; lds_addr_increment
	s_mov_b32 s82, 0x100                               ; global_addr_increment[31:0]
	s_mov_b32 s83, 0x30000                             ; iterate_count=3, increment[47:32]=0
	s_mov_b32 s84, 0                                   ; group3 is unused/zero
	s_mov_b32 s85, 0
	s_mov_b32 s86, 0
	s_mov_b32 s87, 0
	...
	s_or_b32 s67, s26, 0x1000000                               ; 0000000020A4: 8C43FF1A 08000000
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 0000000020C0: D0310000 00100000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 00000000210C: D0310000 00100000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 000000002158: D0310000 00100000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 000000002B08: D0310000 00100000 7C7C2420
...
.Lbranch_000000002b48:
	; v17_v6 wave1 A payload: K[128,256).
	...
	s_lshl_b32 s26, s13, 4                                     ; 000000002BBC: 841A840E
	...
	s_mul_i32 s24, s55, 0x40                                  ; full M64 bounds: split is along K
	s_sub_co_u32 s26, s17, s24                                 ; 000000002BE8: 809A1812
	...
	s_or_b32 s39, s39, 0x1000000                               ; 000000002C18: 8C27FF27 08000000
	s_and_b32 s40, s40, 0xffff0000                             ; 000000002C20: 8B28FF28 FFFF0000
	s_or_b32 s40, s40, 4                                      ; four M16 blocks, 0x400 B per iteration
	s_lshl_b32 s24, s13, 4                                     ; 000000002C2C: 8418840E
	...
	s_mul_i32 s24, s69, 0x40
	s_mul_hi_u32 s63, s24, s13                                 ; 000000002CBC: 96BF0E18
	s_mul_i32 s24, s24, s13                                    ; 000000002CC0: 96180E18
	s_add_co_u32 s62, s4, s24                                  ; 000000002CC4: 803E1806
	s_add_co_ci_u32 s63, s63, s5                               ; 000000002CC8: 823F073F
	s_add_co_u32 s62, s62, 0x400                               ; next task, physical K128
	...
	s_mul_i32 s27, s69, 0x40                                  ; next task also keeps full M64 bounds
	s_sub_co_u32 s27, s17, s27                                 ; 000000002CEC: 809B1B12
	...
	s_or_b32 s67, s26, 0x1000000                               ; 000000002D08: 8C43FF1A 08000000
	...
	s_set_vgpr_msb 0
	s_mov_b32 s33, 0x1000
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000003530: D0310000 00000000 7C7C2420
	...
	s_mov_b32 s33, 0x3000
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 00000000357C: D0310000 00000000 7C7C2420
	...
	s_mov_b32 s33, 0x5000
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 0000000035C8: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 00000000376C: D0310000 00000000 7C7C2420
...
.Lbranch_0000000037ac:
	; v17_v6 wave2 B payload: K[0,128).
	...
	s_lshl_b32 s26, s14, 4                                     ; 000000003820: 841A850F
	...
	s_mul_i32 s24, s54, 0x100
	s_sub_co_u32 s26, s18, s24                                 ; 00000000384C: 809A1811
	s_lshr_b32 s26, s26, 4                                     ; 000000003850: 851A851A
	...
	s_or_b32 s39, s39, 0x1000000
	s_and_b32 s40, s40, 0xffff0000                             ; 000000003884: 8B28FF28 FFFF0000
	s_or_b32 s40, s40, 16                                     ; sixteen N16 blocks, 0x1000 B per iteration
	s_lshl_b32 s24, s14, 4                                     ; 000000003890: 8418850F
	...
	s_mov_b32 s53, 1                                           ; SA multicast: all four N WGs in the M row
	...
	; D# group2/group3: LDS +0x1000, global +0x100, four iterations.
	s_bitset1_b32 s36, 19                              ; iterate_enable
	s_mov_b32 s80, 0                                   ; tensor_dim2
	s_mov_b32 s81, 0x1000                             ; lds_addr_increment
	s_mov_b32 s82, 0x100                               ; global_addr_increment[31:0]
	s_mov_b32 s83, 0x30000                             ; iterate_count=3, increment[47:32]=0
	s_mov_b32 s84, 0                                   ; group3 is unused/zero
	s_mov_b32 s85, 0
	...
	s_mul_hi_u32 s63, s24, s14                                 ; 000000003904: 96BF0F18
	s_mul_i32 s24, s24, s14                                    ; 000000003908: 96180F18
	s_add_co_u32 s62, s6, s24                                  ; 00000000390C: 803E1808
	s_add_co_ci_u32 s63, s63, s7                               ; 000000003910: 823F093F
	...
	s_sub_co_u32 s27, s18, s27                                 ; 000000003934: 809B1B11
	s_lshr_b32 s27, s27, 4                                     ; 000000003938: 851B851B
	...
	s_nop 0                                                   ; 000000003950: 8C43FF1A 01000000
	...
	s_set_vgpr_msb 0
	s_mov_b32 s33, 0xa800
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 00000000417C: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 0000000041C8: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000004214: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 0000000043B8: D0310000 00000000 7C7C2420
...
.Lbranch_0000000043f8:
	; v17_v6 wave3 B payload: K[128,256).
	s_mov_b32 s95, 0xe800
	s_mov_b32 s96, 0x16800
	s_mov_b32 s97, 0x1e800
	s_mov_b32 s98, 0x26800
	...
	s_lshl_b32 s26, s14, 4                                     ; 00000000446C: 841A8510
	...
	s_mul_i32 s24, s54, 0x100                                 ; full N256 bounds: split is along K
	s_sub_co_u32 s26, s18, s24                                 ; 000000004498: 809A1812
	s_lshr_b32 s26, s26, 4                                     ; 00000000449C: 851A851A
	...
	s_or_b32 s40, s40, 16                                     ; sixteen N16 blocks, 0x1000 B per iteration
	s_lshl_b32 s24, s14, 4                                     ; 0000000044DC: 84188510
	...
	s_mul_i32 s24, s68, 0x100
	s_mul_hi_u32 s63, s24, s14                                 ; 00000000456C: 96BF1018
	s_mul_i32 s24, s24, s14                                    ; 000000004570: 96181018
	s_add_co_u32 s62, s6, s24                                 ; 000000004574: 803E180A
	s_add_co_ci_u32 s63, s63, s7                              ; 000000004578: 823F0B3F
	s_add_co_u32 s62, s62, 0x400                               ; next task, physical K128
	s_add_co_ci_u32 s63, s63, 0
	...
	s_mul_i32 s27, s68, 0x100                                 ; next task also keeps full N256 bounds
	s_sub_co_u32 s27, s18, s27                                 ; 00000000459C: 809B1B12
	s_lshr_b32 s27, s27, 4                                     ; 0000000045A0: 851B851B
	...
	s_nop 0                                                   ; 0000000045B8: 8C43FF1A 01000000
	...
	s_set_vgpr_msb 0
	s_mov_b32 s33, 0xe800
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000004DE4: D0310000 00000000 7C7C2420
	...
	s_mov_b32 s33, 0x16800
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000004E30: D0310000 00000000 7C7C2420
	...
	s_mov_b32 s33, 0x1e800
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000004E7C: D0310000 00000000 7C7C2420
	...
	s_set_vgpr_msb 0
	s_mov_b32 s33, 0x26800
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000005020: D0310000 00000000 7C7C2420
...
.Lbranch_000000005060:
	...
	s_cselect_b32 s24, s4, s24                                 ; 0000000050E4: 98181806
	s_cselect_b32 s25, s5, s25                                 ; 0000000050E8: 98191907
	s_cselect_b32 s26, s13, s26                                ; 0000000050EC: 981A1A0E
	s_cmp_eq_u32 s22, 2                                        ; 0000000050F0: BF068216
	s_cselect_b32 s24, s6, s24                                 ; 0000000050F4: 98181808
	s_cselect_b32 s25, s7, s25                                 ; 0000000050F8: 98191909
	s_cselect_b32 s26, s14, s26                                ; 0000000050FC: 981A1A0F
	s_cmp_eq_u32 s22, 3                                        ; 000000005100: BF068316
	s_cselect_b32 s24, s6, s24                                ; 000000005104: 9818180A
	s_cselect_b32 s25, s7, s25                                ; 000000005108: 9819190B
	s_cselect_b32 s26, s14, s26                                ; 00000000510C: 981A1A10
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 0000000052CC: D0310000 00100000 7C7C2420
	...
	s_mov_b32 s33, 0x2000
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 000000005318: D0310000 00100000 7C7C2420
	...
	s_mov_b32 s33, 0x4000
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 000000005364: D0310000 00100000 7C7C2420
	...
	s_set_vgpr_msb 0
	s_mov_b32 s33, 0x6000
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 000000005508: D0310000 00100000 7C7C2420
...
.Lbranch_00000000554c:
	...
	s_cselect_b32 s24, s4, s24                                 ; 0000000055CC: 98181806
	s_cselect_b32 s25, s5, s25                                 ; 0000000055D0: 98191907
	s_cselect_b32 s26, s13, s26                                ; 0000000055D4: 981A1A0E
	s_cmp_eq_u32 s22, 2                                        ; 0000000055D8: BF068216
	s_cselect_b32 s24, s6, s24                                 ; 0000000055DC: 98181808
	s_cselect_b32 s25, s7, s25                                 ; 0000000055E0: 98191909
	s_cselect_b32 s26, s14, s26                                ; 0000000055E4: 981A1A0F
	s_cmp_eq_u32 s22, 3                                        ; 0000000055E8: BF068316
	s_cselect_b32 s24, s6, s24                                ; 0000000055EC: 9818180A
	s_cselect_b32 s25, s7, s25                                ; 0000000055F0: 9819190B
	s_cselect_b32 s26, s14, s26                                ; 0000000055F4: 981A1A10
	...
	s_mov_b32 s33, 0xa800
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 0000000057D8: D0310000 00000000 7C7C2420
	...
	s_mov_b32 s33, 0x12800
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000005824: D0310000 00000000 7C7C2420
	...
	s_mov_b32 s33, 0x1a800
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000005870: D0310000 00000000 7C7C2420
	...
	s_set_vgpr_msb 0
	s_mov_b32 s33, 0x22800
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000005A14: D0310000 00000000 7C7C2420
...
.Lbranch_000000005a58:
	...
	s_nop 0                                                   ; 000000005AAC: 8C43FF1A 01000000
	...
	s_cselect_b32 s24, s4, s24                                 ; 000000005AD8: 98181806
	s_cselect_b32 s25, s5, s25                                 ; 000000005ADC: 98191907
	s_cselect_b32 s26, s13, s26                                ; 000000005AE0: 981A1A0E
	s_cmp_eq_u32 s22, 2                                        ; 000000005AE4: BF068216
	s_cselect_b32 s24, s6, s24                                 ; 000000005AE8: 98181808
	s_cselect_b32 s25, s7, s25                                 ; 000000005AEC: 98191909
	s_cselect_b32 s26, s14, s26                                ; 000000005AF0: 981A1A0F
	s_cmp_eq_u32 s22, 3                                        ; 000000005AF4: BF068316
	s_cselect_b32 s24, s6, s24                                ; 000000005AF8: 9818180A
	s_cselect_b32 s25, s7, s25                                ; 000000005AFC: 9819190B
	s_cselect_b32 s26, s14, s26                                ; 000000005B00: 981A1A10
	...
	s_nop 0                                                   ; 000000005C40: 8C27FF27 01000000
	...
	s_mov_b32 s33, 0x8000
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000005CC8: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000005D14: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000005D60: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000005F04: D0310000 00000000 7C7C2420
...
.Lbranch_000000005f48:
	...
	s_nop 0                                                   ; 000000005F9C: 8C43FF1A 01000000
	...
	s_cselect_b32 s24, s4, s24                                 ; 000000005FC8: 98181806
	s_cselect_b32 s25, s5, s25                                 ; 000000005FCC: 98191907
	s_cselect_b32 s26, s13, s26                                ; 000000005FD0: 981A1A0E
	s_cmp_eq_u32 s22, 2                                        ; 000000005FD4: BF068216
	s_cselect_b32 s24, s6, s24                                 ; 000000005FD8: 98181808
	s_cselect_b32 s25, s7, s25                                 ; 000000005FDC: 98191909
	s_cselect_b32 s26, s14, s26                                ; 000000005FE0: 981A1A0F
	s_cmp_eq_u32 s22, 3                                        ; 000000005FE4: BF068316
	s_cselect_b32 s24, s6, s24                                ; 000000005FE8: 9818180A
	s_cselect_b32 s25, s7, s25                                ; 000000005FEC: 9819190B
	s_cselect_b32 s26, s14, s26                                ; 000000005FF0: 981A1A10
	...
	s_nop 0                                                   ; 000000006130: 8C27FF27 01000000
	...
	s_mov_b32 s33, 0x8800
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 0000000061D4: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000006220: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 00000000626C: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000006410: D0310000 00000000 7C7C2420
...
.Lbranch_00000000647c:
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 000000006480: D0310000 00100000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 0000000064C8: D0310000 00100000 7C7C2420
...
.Lbranch_000000006510:
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 000000006514: D0310000 00100000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 00000000655C: D0310000 00100000 7C7C2420
...
.Lbranch_0000000065a4:
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 0000000065A8: D0310000 00100000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 0000000065F0: D0310000 00100000 7C7C2420
...
.Lbranch_000000006638:
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 00000000663C: D0310000 00100000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT        ; 000000006684: D0310000 00100000 7C7C2420
...
.Lbranch_0000000066f4:
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 0000000066F8: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000006740: D0310000 00000000 7C7C2420
...
.Lbranch_000000006788:
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 00000000678C: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 0000000067D4: D0310000 00000000 7C7C2420
...
.Lbranch_00000000681c:
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000006820: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 000000006868: D0310000 00000000 7C7C2420
...
.Lbranch_0000000068b0:
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 0000000068B4: D0310000 00000000 7C7C2420
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]                      ; 0000000068FC: D0310000 00000000 7C7C2420
...
.Lbranch_000000007158:
	...
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87] th:TH_LOAD_NT
...
.Lbranch_000000008e6c:
	...
	s_mov_b32 s33, s95
	s_barrier_wait 0xffff
	tensor_load_to_lds s[32:35], s[36:43], s[80:83], s[84:87]
	s_add_co_u32 s24, s58, 0x500
	s_cmp_lt_u32 s24, s70
	s_cselect_b64 s[34:35], s[34:35], s[62:63]
	s_cselect_b32 s36, s36, s64
